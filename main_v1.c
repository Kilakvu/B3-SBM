...
void app_main(void *arg) {
    ...
    sprintf(lines[1], "Esperando...");
    actualizar(lines);

    uint32_t last_event_tick = osKernelGetTickCount();
    int showing_waiting = 1; 
    ...
    while (1) {
        ...
        if (events) {
            ...
            else if (events & JOY_EVENT_DOWN)  sprintf(lines[0], "ABAJO");
            else if (events & JOY_EVENT_LEFT)  sprintf(lines[0], "IZQUIERDA");
            else if (events & JOY_EVENT_RIGHT) sprintf(lines[0], "DERECHA");
            else if (events & JOY_EVENT_CENTER) {
            ...
            last_event_tick = osKernelGetTickCount();
            showing_waiting = 0;
            
        } 
        else if (!showing_waiting && (osKernelGetTickCount() - last_event_tick > 1000)) {
            lcd_clean();
            sprintf(lines[0], "Test LCD+Joystick");
            sprintf(lines[1], "Esperando...");
            actualizar(lines);
